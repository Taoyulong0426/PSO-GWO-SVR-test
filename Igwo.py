    ...
    def sanitized_gwo(self):
        ...
        for i in range(0, self.SearchAgents_no):
            for j in range(0, self.dim):
                if i ==0:
                    Positions[i,j]=np.random.uniform(low=self.lb[j],high=self.ub[j])
                else:
                    Positions[i, j] = (4 * Positions[i - 1][j] + 0.5 - 0.8 / (2 * math.pi) * math.sin(
                        2 * math.pi * Positions[i - 1][j])) % 1 * (self.ub[j]-self.lb[j])
        # 初始化ABD狼的位置
        ...
        while t < self.T:
            ...
            a = 2 - math.cos((math.pi / 2) * (t / self.T))
